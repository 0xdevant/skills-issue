...
```text
Install the "token-usage-audit" skill from https://github.com/0xdevant/skills-issue
...
rsync -a --delete skills-issue/token-usage-audit/ <your-skills-dir>/token-usage-audit/
...
```bash
cd skills-issue/token-usage-audit && node scripts/audit.mjs --since 30d
```
...
GPL-3.0-or-later. See the repo [LICENSE](../LICENSE).
